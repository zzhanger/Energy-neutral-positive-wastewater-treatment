...
for i, col in enumerate(target_columns):
    ...
    if len(data) > 0:
        ...
        if i == 0:
            ...
            axes[i].plot(x_vals, y_vals, '#344966', linewidth=2, alpha=0.8)
        ...
        axes[i].set_xlabel(display_name, fontsize=24)
        axes[i].set_ylabel('Probability density', fontsize=24)
        axes[i].set_title(f'{display_names_no_units[col]} distribution', fontsize=24, fontweight='bold')
...
# Create joint plot using seaborn with larger figure size
g = sns.jointplot(
    ...
    height=16,  # 进一步增大画布高度
    ratio=4,    # 调整主图与边缘图的比例
    space=0.1,  # 调整主图与边缘图的间距
    joint_kws={'s': 60},  # 增大散点大小
...
y_line = x_line
g.ax_joint.plot(x_line, y_line, '#48B3AF', linewidth=3, label='Energy production from wastewater treatment (Y=X)')

# Add y=0.5x line (50% energy coverage line)
y_half_line = 0.5 * x_line
g.ax_joint.plot(x_line, y_half_line, '#A7E399', linewidth=3, label='50% energy self sufficient (Y=0.5X)')
...
# 调整标记文字位置，避免被图例遮挡
g.ax_joint.text(0.3 * max_val, 0.45 * max_val, f'{above_y_equal_x} facilities', 
         fontsize=32, bbox=dict(boxstyle='round', facecolor='w', alpha=0.8),
         ha='center', va='center')
g.ax_joint.text(0.85 * max_val, 0.5 * max_val, f'{between_lines} facilities', 
         fontsize=32, bbox=dict(boxstyle='round', facecolor='#A7E399', alpha=0.8),
         ha='center', va='center')
g.ax_joint.text(0.85 * max_val, 0.15 * max_val, f'{below_y_equal_half_x} facilities', 
         fontsize=32, bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.8),
         ha='center', va='center')

# Add shaded regions
g.ax_joint.fill_between(x_line, y_line, max_val, alpha=0.2, color='#48B3AF', label='Energy surplus region')
g.ax_joint.fill_between(x_line, y_half_line, y_line, alpha=0.2, color='#A7E399', label='50-100% energy self sufficient')
g.ax_joint.fill_between(x_line, 0, y_half_line, alpha=0.2, color='yellow', label='<50% energy self sufficient')

# Set labels for main plot with text wrapping
x_label = "Adjusted energy use intensity after removing technical inefficiency (kWh/m$^3$)"
y_label = "Recoverable energy density (kWh/m³)"

# 自动换行处理
wrapped_x_label = textwrap.fill(x_label, width=40)  # 每行最多40个字符
wrapped_y_label = textwrap.fill(y_label, width=30)  # 每行最多30个字符

g.ax_joint.set_xlabel(wrapped_x_label, fontsize=32)
g.ax_joint.set_ylabel(wrapped_y_label, fontsize=32)

# 增大坐标轴刻度字体大小
g.ax_joint.tick_params(axis='both', which='major', labelsize=32)
g.ax_joint.tick_params(axis='both', which='minor', labelsize=32)

# 增大边缘图刻度字体大小
g.ax_marg_x.tick_params(axis='x', which='major', labelsize=32)
g.ax_marg_y.tick_params(axis='y', which='major', labelsize=32)

g.ax_joint.grid(True, alpha=0.3)

# 设置图例：白色背景框，调整位置避免遮挡第一个区域的数量
legend = g.ax_joint.legend(
    ...
    facecolor='white', 
    edgecolor='gray',
    loc='upper left',  # 将图例移到左上角
    bbox_to_anchor=(0.02, 0.98)  # 微调位置，避免遮挡
)
...
g.ax_marg_x.axvline(x_mean, color='#333333', linestyle='--', linewidth=3, label=f'Mean: {x_mean:.2f}')
g.ax_marg_y.axhline(y_mean, color='#333333', linestyle='--', linewidth=3, label=f'Mean: {y_mean:.2f}')
...
for i, process in enumerate(process_types):
    ...
    if len(process_data) > 0:
        ...
        plt.scatter(x_process, y_process, alpha=0.7, color=colors[i], marker=markers[i], 
                   s=120, label=f'{process_names[process]}', edgecolors='white', linewidth=1)  # 增大散点大小

# Add y=x line (Energy production from wastewater treatment)
max_val = max(x_data.max(), y_data.max())
x_line = np.linspace(0, max_val, 100)
y_line = x_line
plt.plot(x_line, y_line, 'k--', linewidth=3, label='Energy production from wastewater treatment (Y=X)')  # 增大线条宽度

# Add y=0.5x line (50% energy self-sufficient)
y_half_line = 0.5 * x_line
plt.plot(x_line, y_half_line, 'k:', linewidth=3, label='50% energy self-sufficient (Y=0.5X)')  # 增大线条宽度
...
# Add text annotations with counts for each region and process type - 增大字体
# Above Y=X region
...
plt.text(0.7 * max_val, 0.85 * max_val, above_text, 
         fontsize=24, bbox=dict(boxstyle='round', facecolor='#476EAE', alpha=0.2),  # 增大字体
         ha='center', va='center')
...
plt.text(0.7 * max_val, 0.5 * max_val, between_text, 
         fontsize=24, bbox=dict(boxstyle='round', facecolor='#48B3AF', alpha=0.2),  # 增大字体
         ha='center', va='center')
...
plt.text(0.7 * max_val, 0.15 * max_val, below_text, 
         fontsize=24, bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.2),  # 增大字体
         ha='center', va='center')

# Add total sustainable scenarios count on the plot - 增大字体
plt.text(0.05 * max_val, 0.95 * max_val, f'Total facilities capable\nof sustainable pathways: {total_points}\n(excluded facilities achieved energy positive under baseline pathway', 
         fontsize=24, bbox=dict(boxstyle='round', facecolor='white', alpha=0.8),  # 增大字体
         ha='left', va='top', transform=plt.gca().transAxes)

# Add shaded regions
plt.fill_between(x_line, y_line, max_val, alpha=0.2, color='#476EAE', label='Energy surplus region')
plt.fill_between(x_line, y_half_line, y_line, alpha=0.2, color='#48B3AF', label='50-100% self-sufficient')
plt.fill_between(x_line, 0, y_half_line, alpha=0.2, color='yellow', label='<50% self-sufficient')

# Set labels and title - 增大字体
plt.xlabel('Energy use intensity of sustainable pathways (kWh/m³)', fontsize=28)  # 增大字体
plt.ylabel('Recoverable energy density of sustainable pathways (kWh/m³)', fontsize=28)  # 增大字体
